...
if ( ! class_exists( '\E20R\Utilities\Licensing\License_Server' ) ) {
	class License_Server {
		 ...
		 * @return bool
		 */
		public static function get_license_status_from_server( $product, $settings = null, $force = false ) {
			...
			if ( ! Licensing::is_new_version() && empty( $settings['key'] ) ) {
				if ( E20R_LICENSING_DEBUG ) {
					$utils->log( "Old store: {$product} has no key stored. Returning false" );
				}
			...
			if ( Licensing::is_new_version() && empty( $settings['the_key'] ) ) {
				if ( E20R_LICENSING_DEBUG ) {
					$utils->log( "New store: {$product} has no key stored. Returning false" );
				}
			...
			if ( ( true === $force || true === E20R_LICENSING_DEBUG ) || false === ( $license_status = (bool) Cache::get( "{$product}_status", 'e20r_licensing' ) ) ) {
