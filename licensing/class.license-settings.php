...
class License_Settings {
	...
	private function __construct() {
		...
		if ( ! defined( 'E20R_LICENSE_SERVER_URL' ) ||
		     ( defined( 'E20R_LICENSE_SERVER_URL' ) && empty( E20R_LICENSE_SERVER_URL ) )
		) {
			...
			if ( ! in_array( $k, array(
				...
				if ( 'active' === $license['status'] ) {
					...
					$is_active   = Licensing::is_active(
						( isset( $license['license_key'] ) && ! Licensing::is_new_version() ?
							$license['license_key'] :
	 ...
	 * @return array
	 */
	public static function get_settings( $product = null ) {
	 ...
	 * @return array
	 */
	public static function default_settings( $product = 'e20r_default_license' ) {
	 ...
	 * @return array
	 */
	public static function merge_settings( $product, $new_settings ) {
		
		$utils        = Utilities::get_instance();
		$old_settings = self::get_settings( $product );
		
		if ( E20R_LICENSING_DEBUG ) {
			$utils->log( "Previously saved settings for {$product}: " . print_r( $old_settings, true ) );
	 ...
	 * @return bool|array
	 */
	public static function update_settings( $product = null, $new_settings ) {
