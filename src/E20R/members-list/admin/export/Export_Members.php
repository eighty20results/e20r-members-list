...
if ( ! class_exists( '\\E20R\\Members_List\\Admin\\Export\\Export_Members' ) ) {
	...
	class Export_Members {
		...
		/**
		 * The primary PMPro Membership Level name/id
		 *
		 * @var string|int|null $primary_level_id
		 */
		private $primary_level_id = null;

		/**
		 * Array of Membership IDs that the user has been assigned
		 *
		 * @var array $level_id_array
		 */
		private $level_id_array = array();

		/**
		 * Export_Members constructor.
		 *
		 * @param stdClass[]     $db_records The array of records to export.
		 * @param Utilities|null $utils      An instance of the E20R Utilities Module class
		 */
		public function __construct( $db_records, $utils = null ) {
		...
		private function csv_header() {
			...
			if ( empty( $this->primary_level_id ) ) {
				$this->primary_level_id = $this->utils->get_variable( 'membership_id', '' );
			}

			if ( empty( $this->level_id_array ) ) {
				$level_id_list        = $this->utils->get_variable( 'membership_ids', '' );
				$this->level_id_array = array_map( 'trim', explode( ',', $level_id_list ) );
			}
			...
			$this->utils->log( 'Using ' . count( $this->csv_headers ) . ' header columns' );
		}
		 ...
		 */
		public static function clear_temp_files( $utils = null ) {
		...
		public function get_data_list() {
			...
			$this->utils->log( 'Headers have been sent already..?!? ' . ( headers_sent() ? 'Yes' : 'No' ) );
			...
			for ( $ic = 1; $ic <= $iterations; $ic ++ ) {
				...
				$this->utils->log( "For iterations: {$i_start} -> " . ( $i_start + $max_users_per_loop ) );

				$member_list = array_slice( $this->member_list, $i_start, $max_users_per_loop );

				$this->utils->log( 'Will process ' . count( $member_list ) . " member records in iteration {$ic}" );
		...
		private function add_csv_header_to_file() {
			...
			$this->utils->log( 'Adding ' . count( $this->csv_headers ) . " header columns to {$this->file_name}" );
		...
		private function map_keys( $key, $requested, $column_type = null ) {
			...
			$this->utils->log( "No value (key) found for {$requested} key {$key}" );
		...
		public function load_export_value( $csv_record, $member ) {
			...
			foreach ( $this->default_columns as $field_def ) {
				...
				if ( 'enddate' === $column_name ) {
					...
					if ( ! is_null( $member->membership_id ) && ! empty( $column_value ) ) {
						...
						if (
							in_array( $this->primary_level_id, array( 'oldmembers', 'expired', 'cancelled' ), true ) &&
				...
				if ( ! empty( $member->code_id ) && empty( $this->member_discount_info ) ) {
					...
					$this->utils->log( "Grab the discount code data for {$member->ID}/{$member->code_id}" );
			...
			// Clear the member specific information
			$this->member_discount_info = null;
			$this->level_id_array       = array();
			$this->primary_level_id     = null;
		...
		private function get_column_value( $member, $column_name, $column_type ) {
			...
			switch ( $column_type ) {
				...
				default:
					$this->utils->log( "Using default type (type = {$column_type}) for {$column_name}" );
		...
		public function save_data_for_export() {
			...
			$this->utils->log( 'Saving ' . count( $this->csv_rows ) . " records to {$this->file_name}. " );
			...
			foreach ( $this->csv_rows as $row_id => $row_data ) {
				...
				$file_line = implode( ',', $data ) . PHP_EOL;
				fprintf( $fh, '%s', $file_line );
			}
			// phpcs:ignore
			fclose( $fh );
			$this->utils->log( "Saved data to {$this->file_name}" );
		}
		...
		public function return_content() {
			...
			if ( empty( $this->headers ) ) {
				...
				$this->utils->log( $msg . print_r( ob_get_contents(), true ) );
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			$this->utils->log( 'DEBUG: Browser received: ' . print_r( $sent_content, true ) );
